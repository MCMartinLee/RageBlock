import type { EnemyArchetype } from "./prototype/enemyArchetypes";
import type { ChapterHazardKind } from "./prototype/chapterHazards";

export type CampaignEnemyArchetype = Exclude<EnemyArchetype, "boss">;
export type CampaignChapterId = "back-lot" | "arcade-strip" | "apartment-maze" | "canal-walk" | "community-fair" | "rooftop-relay";
...
export function validateCampaignChapters(chapters: CampaignChapter[] = CAMPAIGN_CHAPTERS): boolean {
  ...
  return chapters.length === 6 && new Set(ids).size === ids.length && new Set(chapters.map((chapter) => chapter.faction)).size === chapters.length && chapters.every((chapter) => chapter.enemyRoster.length > 0 && chapter.reward.length > 0 && chapter.route.some((node) => node.kind === "side") && chapter.route.some((node) => node.kind === "climax") && chapter.hazards.length > 0);
}
