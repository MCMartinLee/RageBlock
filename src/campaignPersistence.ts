import { createCampaignState, type CampaignState, type RageMode } from "./campaignRuntime";
...
export function loadCampaign(storage: CampaignStorage): CampaignState {
  try {
    ...
    if (!parsed || typeof parsed.chapterIndex !== "number") return createCampaignState(loadSelectedMode(storage));
    return { ...createCampaignState(loadSelectedMode(storage)), ...parsed };
  } catch {
    return createCampaignState(loadSelectedMode(storage));
  }
}
